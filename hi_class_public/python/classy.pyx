...
cdef class Class:
    ...
    def growthrate_at_k_and_z(self,k,z):
        dz = 0.005
        f=-0.5*(1+z)*(-3.*np.log(self.pk(k,z))+4.*np.log(self.pk(k,z+dz))-np.log(self.pk(k,z+2.*dz)))/(2.*dz)
        if (f==0.):
            raise CosmoSevereError(
            "The step in redshift is too small to compute growth rate")
        return f
    
    def growthrate_at_z(self,z):
        k_fid = 0.01
        return self.growthrate_at_k_and_z(k_fid,z)
    
    def sigma8_at_z(self,z):
        R = 8./self.h()
        return self.sigma_at_r_and_z(R,z)

    def sigma_at_r_and_z(self,double R,double z):
    ...
    def linear_growth_rate(self, z):
        ...
        f_z = pvecback[self.ba.index_bg_f]
    ...
    def linear_growth_factor(self, z):
        ...
        D_z = pvecback[self.ba.index_bg_D]
